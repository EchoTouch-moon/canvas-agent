...
## CR-002 — ModelCallObservation and Context Universe Research Model
...
- repeated context items can be correlated by content hash / source identity;
- Snapshot items and Run-derived observations are distinguishable;
...
## CR-003 — Shadow Working Set Evaluator
...
REMOVE
- grep-result-8
  superseded by diagnosis-3

REPLACE
- full auth.ts
+ refreshSession symbol
...
- context items removed and later observed as needed again;
...
- every removal has an inspectable reason;
- false-removal candidates can be identified;
- policy output is deterministic for the same normalized inputs.
...
- tool-call continuity must be preserved;
- system / high-authority instructions cannot be silently removed;
...
- all active removals are observable in the timeline;
...
explainability
- transitions with explicit reasons
...
- reuse the same Runtime core and policy definitions from Pi;
...
- a common `ContextCandidate / WorkingSet / Transition` model can represent both harnesses;
...
- ADD / REMOVE / REPLACE / REHYDRATE diff;
- decision reason;
- provenance;
...
3. a deterministic Shadow Working Set policy;
4. at least one real Dynamic rewrite experiment;
5. ContextTransition evidence for every rewrite;
6. measured quality and context-efficiency results;
7. cross-model evidence from at least two model families, or an explicit documented reason this was not possible;
8. a second-harness portability result or a documented abstraction failure;
9. a decision on whether Codex / closed-Agent compatibility is worth pursuing next;
10. a decision on whether Context Runtime should remain inside Canvas Agent or begin extracting into an independently consumable package / service.
