...
- 授权 runner 绑定到 clean implementation checkout `0e55a36f3a2c47bef9d90611df1c75a389a9263d`；execution hash=`f5b5f10c...`，control hash=`8ac49f85...`，final run-contract hash=`bbb7db4f...`。
...
- Provider key 只能作为内存输入传递；本轮实现和测试没有读取 `.env`。CLI 仅在 exact owner authorization、contract/surface/budget 校验及 identity 未 claim 检查之后，才从进程环境或指定 `.env` 路径读取 key；假 Provider 测试必须断言网络请求为 0。
...
- 真实执行只接受延迟调用的内存 credential reader；`.env` 只会在该 reader 被调用时解析，值不会写入 evidence。Node test 环境必须显式注入 fake fetch；生产入口拒绝注入 transport，默认真实 fetch 仍需单独 owner authorization。
