...
`C1LiveBindingDriver` 接到一层极薄的 live binding wiring。此阶段只证明 wiring 和证据边界可执行，
...
1. `c1-e0-live-binding.ts` 复用 `buildC1E0ExecutionPlans`、`C1LiveBindingDriver`、sandbox tool executor、
   checkpoint sink 和 E0 dose/readiness adjudicator。
2. `C1E0NaturalObservationSource` 从 fixture 的真实 read/tool-result 开始；每次成功 edit/write 后以私有
...
6. `C1AuthorizedProviderResponseSource` 增加可选 outer request hash override，使 E0 authorized adapter 能
   保留 strict profile binding，同时验证 E0 request hash。

## 验证矩阵

- 8 legs / 4 pairs / t1×2 / t2×2，2:2 arm order，所有 leg 使用 fresh fixture 和独立 run identity。
- scripted substitute 的 full study：自然 lifecycle、REMOVE dose、Layer 2 两层 oracle、checkpoint join、
  batch qualification PASS，且 `providerCalls=0`、`networkRequests=0`。
...
- 新 PR 保持 Draft，标记为 `documentation`、`enhancement`、`area:context-runtime`，并在 verification
  记录中明确 `READY_FOR_INDEPENDENT_REVIEW`、`finalBindingReady=false`、`Owner authorization=NO_GO`。
