...
| F1 Native Feasibility Frontier | `DRAFT / DESIGN_ONLY / NO_PROVIDER`                                                             | 固定 task/prompt/provider/tool/recovery/oracle，只改变 Provider-call budget；24-call F0-v2 作为历史锚点，候选新 point 为 32/40/48，预先声明 sequential stopping；不创建 identity，不执行 Provider；[设计提案](../plan/c1-f1-native-feasibility-frontier-design-2026-09-13.zh-CN.md) |
| F1-32 machine contract          | `FREEZE_REVIEW / NO_PROVIDER / NO_IDENTITY`                                                     | 只冻结首个 32-call point；study Provider ceiling 由 32 runs × 32 calls 派生为 1024；继承 F0-v2 task/evidence/firewall/recovery/gate 语义，并携带 historical-anchor surface-equivalence witness；execution binding 与 identity 仍未创建；[合同候选 Freeze Review](../verification/cspv-c1-f1-32-contract-freeze-review-2026-09-13.zh-CN.md) |
| F1-32 credential-free runner   | `IMPLEMENTED / CREDENTIAL_FREE_E2E_PASS / BINDING_READY / NO_PROVIDER`                         | 独立 runner 使用 32/run、1024/study 硬预算，实际生成 282-file execution inventory 与 4-file binding-control inventory（含 inherited carried-removals dependency），逐项重算 hash/aggregate；authoritative execution revision 使用 clean checkout 的 `git rev-parse HEAD`，surface 最近变更点单独记录；内存生成 FINAL_BOUND witness/contract；runner E2E、benchmark 与 headless checks 通过；identity 未创建、Provider 未执行；[runner integration](../verification/cspv-c1-f1-32-runner-integration-2026-09-14.zh-CN.md) |
