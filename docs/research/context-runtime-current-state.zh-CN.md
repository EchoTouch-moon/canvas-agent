...
更新时间：2026-09-09（Asia/Shanghai）。前次更新在 `codex/qwen-sv1-response-evidence`（基于 `0120932`），
...
E0 effectiveness contract DESIGN_GO / execution NO_GO
...
- 已起草 `C1 SUPERSEDED_VERSION Effectiveness E0 Contract`：2–4 对资格实验，使用历史机会证据做 enrollment，
  Runtime policy 只使用执行时 evidence，按 treatment integrity、task correctness、efficiency 三层记录，并新增
  `treatmentExposureRatio` 与 `staleEvidenceExposurePrevented`。E0 目前仅 `DESIGN_GO`，未生成 run authorization，
  64-leg 仍为 `NO_GO`。
...
| E0 Effectiveness Contract | `DESIGN_GO / EXECUTION_NO_GO` | [E0 合同草案](../plan/c1-superseded-version-effectiveness-e0-contract-2026-09-09.zh-CN.md)；2–4 matched pairs；先冻结 enrollment manifest / run contract / dose schema / review / authorization，再决定是否执行 |
