...
F0-v2 contract design    REVISED / READY_FOR_CONTRACT_FREEZE_REVIEW / NO_PROVIDER
...
| F0-v2 contract design            | `REVISED / READY_FOR_CONTRACT_FREEZE_REVIEW / NO_PROVIDER` | 保持 F0-v1 task、prompt、Provider、24-call 预算和分层 gate；补齐 post-run snapshot→adjudication→cleanup 顺序、UNKNOWN 分层与 hardening recovery 语义；新 contract、execution revision、surface hash 与 study identity 均待 freeze；[设计提案](../plan/c1-f0-v2-recovery-validation-design-2026-09-13.zh-CN.md)                                                                                                             |
