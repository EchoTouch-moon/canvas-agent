...
import {
  ...
  type C1LifecycleCanaryAuthorization
...
if ((mode !== '--fake' && mode !== '--live') || !authorizationPath || !outputRoot) {
  throw new Error('Usage: c1-lifecycle-canary.ts --fake|--live binding.json output-directory')
}
const binding = JSON.parse(
  await readFile(authorizationPath, 'utf8')
) as C1LifecycleCanaryAuthorization
...
process.stdout.write(
  JSON.stringify({
    ...
    finalStage: report.finalStage,
    changedCalls: report.changedCalls
  }) + '\n'
)
