  ...
  return {
    ...
    summary() {
      const rows = this.lines.map((line) => {
        const lineText = formatPrice(this.lineTotal(line), this.currency)
        return `  ${line.name} x${line.quantity} ${lineText}`
      })
      const subtotalText = formatPrice(this.subtotal(), this.currency)
      const roundedText = formatPrice(this.subtotal(), this.currency)
