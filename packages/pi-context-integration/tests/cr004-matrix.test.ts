...
describe('MX run identity and leg order', () => {
  it('accepts exactly the REGISTERED series (m1..m9); unregistered m10+ are refused', () => {
    ...
    // M6, M7, M8, and M9 became deliberate with their pre-registered contracts).
    ...
    expect(isValidMxRunId('cr004-m8-20260830-4d7e9a1b')).toBe(true)
    expect(isValidMxRunId('cr004-m9-20260830-4d7e9a1b')).toBe(true)
    expect(isValidMxRunId('cr004-m9-20260901-4d7e9a1b')).toBe(true)
    expect(isValidMxRunId('cr004-m10-20260901-4d7e9a1b')).toBe(false)
    ...
    // Suggestions come from the LATEST registered profile (M9 V3 direct-exposure screen).
    for (let index = 0; index < 8; index += 1) {
      const suggested = suggestMxRunId()
      expect(MX_RUN_ID_PATTERN.test(suggested)).toBe(true)
      expect(/^cr004-m9-\d{8}-[0-9a-f]{8}$/.test(suggested)).toBe(true)
    }
