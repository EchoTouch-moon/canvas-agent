...
describe('MX run identity and leg order', () => {
  it('accepts exactly the REGISTERED series (m1..m8); unregistered m9+ are refused', () => {
    ...
    // M6, M7, and M8 became deliberate with their pre-registered contracts).
    ...
    // Suggestions come from the LATEST registered profile (M8 L3 lifecycle screen).
    for (let index = 0; index < 8; index += 1) {
      const suggested = suggestMxRunId()
      expect(MX_RUN_ID_PATTERN.test(suggested)).toBe(true)
      expect(/^cr004-m8-\d{8}-[0-9a-f]{8}$/.test(suggested)).toBe(true)
    }
