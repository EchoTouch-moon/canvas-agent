...
describe('MX_EXPERIMENT_PROFILES registry', () => {
  it('registers M1..M4 read-only history with distinct contracts and designs', () => {
    expect(MX_EXPERIMENT_PROFILES.map((profile) => profile.series)).toEqual(['M1', 'M2', 'M3', 'M4'])
    ...
    expect(MX_LATEST_PROFILE.series).toBe('M4')
  ...
  it('assertMxProfileBindable refuses unregistered series with a deliberate-act message', () => {
    expect(() =>
      assertMxProfileBindable('cr004-m5-20260901-00000000', { repoRoot: '/tmp' })
...
describe('mxProvenanceWarnings (manifest vs run-id series)', () => {
  ...
  it('warns on unknown series, missing contract, and out-of-bounds recorded shapes', () => {
    expect(mxProvenanceWarnings({ runId: 'cr004-m5-20260901-00000000' })).toEqual([
      'runId series M5 has no experiment profile registered'
    ])
